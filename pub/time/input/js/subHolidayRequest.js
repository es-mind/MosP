...
function onLoadExtra() {
	...
	if (modeActivateDate == MODE_ACTIVATE_DATE_FIXED) {
		...
		setDisabled("pltEditHolidayRange", true);
	}
}
...
function submitApplication(event, cmd) {
	...
	if(jsModeDifferenceRequest1 == "on") {
		var isPmSubHoliday = false;
		if (getFormValue("pltEditHolidayRange") == "3") {
			isPmSubHoliday = true;
		}
		if(isPmSubHoliday) {
			// 入力チェック
			if (validate(validateTarget, null, event)) {
				// 更新系確認メッセージ
			...
			submitRegist(event, validateTarget, null, cmd);
		}
	} else {
		submitRegist(event, validateTarget, null, cmd);
	}
...
function checkStatus(aryMessage, event, status) {
	...
	for (var i = 0; i < checkboxLength; i++) {
		...
		// TD要素内A要素群取得
		var aElements = getElementsByTagName(tdElements.item(3), TAG_A);
		// A要素内SPAN要素群取得
