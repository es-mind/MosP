...
public class WorkPlaceRegistBean extends PlatformBean implements WorkPlaceRegistBeanInterface {
	...
	protected void validate(WorkPlaceDtoInterface dto) {
		// 郵便番号1が空でない場合
		if (MospUtility.isEmpty(dto.getPostalCode1()) == false) {
			// エラーメッセージ用の文字列を準備
			StringBuilder sb = new StringBuilder(PfNameUtility.postalCode(mospParams));
			sb.append(PfNameUtility.parentheses(mospParams, PfNameUtility.digit(mospParams, LEN_POSTAL_CODE_1)));
			// 3桁か確認
			checkInputLength(dto.getPostalCode1(), LEN_POSTAL_CODE_1, sb.toString());
		}
		// 郵便番号2が空でない場合
		if (MospUtility.isEmpty(dto.getPostalCode2()) == false) {
			// エラーメッセージ用の文字列を準備
			StringBuilder sb = new StringBuilder(PfNameUtility.postalCode(mospParams));
			sb.append(PfNameUtility.parentheses(mospParams, PfNameUtility.digit(mospParams, LEN_POSTAL_CODE_2)));
			// 4桁か確認
			checkInputLength(dto.getPostalCode2(), LEN_POSTAL_CODE_2, sb.toString());
		}
	}
