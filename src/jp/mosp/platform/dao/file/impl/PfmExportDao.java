...
public class PfmExportDao extends PlatformDao implements ExportDaoInterface {
	 ...
	 * キー。
	 */
	public static final String	KEY_1				= COL_PFM_EXPORT_ID;
	...
	public BaseDto mapping() throws MospException {
		PfmExportDto dto = new PfmExportDto();
		dto.setPfmExportId(getLong(COL_PFM_EXPORT_ID));
		dto.setExportCode(getString(COL_EXPORT_CODE));
		dto.setExportName(getString(COL_EXPORT_NAME));
		dto.setExportTable(getString(COL_EXPORT_TABLE));
	...
	public List<ExportDtoInterface> mappingAll() throws MospException {
		List<ExportDtoInterface> all = new ArrayList<ExportDtoInterface>();
		while (next()) {
	...
	@Override
	public List<ExportDtoInterface> findForSearch(Map<String, Object> param) throws MospException {
		try {
			// パラメータインデックス準備
			index = 1;
			// 検索条件準備
			String exportCode = getSearchParam(param, SEARCH_EXPORT_CODE);
			String exportName = getSearchParam(param, SEARCH_EXPORT_NAME);
			...
			sb.append(deleteFlagOff());
			sb.append(and());
			sb.append(like(COL_EXPORT_CODE));
			sb.append(and());
			sb.append(like(COL_EXPORT_NAME));
			if (tableType.isEmpty() == false) {
				sb.append(and());
				sb.append(equal(COL_EXPORT_TABLE));
			}
			...
			prepareStatement(sb.toString());
			// パラメータ設定
			setParam(index++, startWithParam(exportCode));
			setParam(index++, containsParam(exportName));
			if (tableType.isEmpty() == false) {
	...
	public int update(BaseDtoInterface baseDto) throws MospException {
		try {
			...
			ExportDtoInterface dto = castDto(baseDto);
			setParam(index++, dto.getPfmExportId());
			executeUpdate();
			chkUpdate(1);
			return cnt;
		} catch (Throwable e) {
	...
	public void setParams(BaseDtoInterface baseDto, boolean isInsert) throws MospException {
		...
		setParam(index++, dto.getExportName());
		setParam(index++, dto.getExportTable());
	 ...
	 * @return キャストされたDTO
	 */
	protected ExportDtoInterface castDto(BaseDtoInterface baseDto) {
		return (ExportDtoInterface)baseDto;
	}
